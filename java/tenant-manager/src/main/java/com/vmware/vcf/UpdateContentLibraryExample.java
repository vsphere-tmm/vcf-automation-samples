...
public class UpdateContentLibraryExample {
    ...
    public static void updateContentLibrary() throws Exception {
        final List<EntityReference> currentStorageClasses = contentLibrary.getStorageClasses();
        if (region2 != null) {
            final List<StorageClass> region2StorageClasses = VcfUtils.getStorageClassesForRegion(openApiClient, region2);
            final Iterator<StorageClass> region2StorageClassesIterator = region2StorageClasses.iterator();
            final StorageClass storageClass = region2StorageClassesIterator.next();
            currentStorageClasses.add(
                    new EntityReference()
                            .id(storageClass.getId())
                            .name(storageClass.getName()));
            System.out.println("Adding storage class " + storageClass.getName() + " from region "
                    + region2.getName() + " to content library " + contentLibrary.getId());
        }
        final ContentLibrary updateParams = new ContentLibrary();
        updateParams.setName("Updated-name");
        updateParams.setDescription("Updated description");
        updateParams.setStorageClasses(currentStorageClasses);
        System.out.println("Updating content library [id: " + contentLibrary.getId() + "] to new name: " + updateParams.getName()
                + " and description: " + updateParams.getDescription() + " with storage class count " + updateParams.getStorageClasses().size());
        contentLibraryApi.updateContentLibrary(updateParams, contentLibrary.getId());
        final TaskType task = openApiClient.getLastTask(contentLibraryApi);
        VcfUtils.waitForTaskWithSuccessStatus(URI.create(task.getHref()));
        vcdClient.getTaskMonitor().waitForSuccess(task, LARGE_TASK_TIMEOUT);
        contentLibrary = VcfUtils.getContentLibraryWithName(openApiClient, updateParams.getName());
    }
}
